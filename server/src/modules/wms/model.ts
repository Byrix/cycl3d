...
export const WmsGetSchema = t.Object({
  ...
  crs: t.Optional(
    t.String({
      pattern: "/^EPSG:/i",
      description: "required for version=1.3.0",
    }),
  ),
  srs: t.Optional(
    t.String({
      pattern: "/^EPSG:/i",
      description: "required for all version below 1.3.0",
    }),
  ),
  ...
  bgcolor: t.Optional(
    t.String({
      pattern: "^#[a-f0-9]{6}",
      default: "#FFFFFF",
      description: "a colour defined by an RGB hex code in format #RRGGBB",
    }),
  ),
